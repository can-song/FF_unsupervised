...
def goodness_score(pos_acts, neg_acts, threshold=2):
    ...
    # return torch.sum(1 / ((pos_acts-neg_acts).pow(2).sum(dim=-1)+1e-6))
    # pho = 0.2
    # # beta = 0.00
    # pos_prob = 1 - torch.exp(-pos_acts)
    # neg_prob = 1 - torch.exp(-neg_acts)
    # f = lambda x: -pho * torch.log(x.mean(dim=2, keepdim=True)+1e-12) - (1-pho) * torch.log(1-x.mean(dim=2, keepdim=True)+1e-12)
    # # return torch.sum(f(pos_prob) + f(neg_prob) - torch.log((pos_acts-neg_acts).pow(2)+1e-12))
    # return torch.sum(f(pos_prob) + f(neg_prob) + 1 / (pho*pho*(pos_prob-neg_prob).pow(2).sum(dim=-1)+1e-6) )
    # return torch.sum(pos_acts.abs() + neg_acts.abs() + 1 / ((pos_acts-neg_acts).pow(2).sum(dim=-1)+1e-6))
    # return torch.sum(f(pos_acts) + f(neg_acts) + 1 / (pho*(pos_acts-neg_acts).pow(2).sum(dim=-1)+1e-6) )
    # return torch.sum(f(pos_acts) + f(neg_acts) + 1 / ((pos_acts-neg_acts).pow(2)+1e-6) )
    # return torch.sum(f(pos_acts) + f(neg_acts) - torch.log((pos_acts-neg_acts).pow(2)+1e-12))
    ...
    return torch.sum(pos_acts + neg_acts - torch.log((pos_acts-neg_acts).pow(2)+1e-12))
