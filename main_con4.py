...
def goodness_score(pos_acts, neg_acts, threshold=2):
    ...
    return torch.sum(f(pos_acts) + f(neg_acts) + 1 / ((pos_acts-neg_acts).pow(2).sum(dim=-1)+1e-6) )
...
class Unsupervised_FF(nn.Module):
    def __init__(self, n_layers: int = 4, n_neurons=500, input_size: int = 28 * 28, n_epochs: int = 100,
        ...
        ff_layers = nn.ModuleList([
            FF_Layer(28*28, 1024, n_epochs, bias, device),
            FF_Layer(1024, 512, n_epochs, bias, device),
...
if __name__ == '__main__':
    ...
    unsupervised_ff = Unsupervised_FF(device=device, n_epochs=2000)
    ...
    # unsupervised_ff.train_ff_layers(pos_dataloader, neg_dataloader)
    # unsupervised_ff.load_state_dict(torch.load("mnist_1900.pt"))
    # unsupervised_ff.n_epochs = 20
    # unsupervised_ff.train_last_layer(pos_dataloader)
