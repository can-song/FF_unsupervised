...
def goodness_score(pos_acts, neg_acts, threshold=2):
    ...
    return torch.sum(beta*f(pos_acts) + beta*f(neg_acts) - (pos_acts-neg_acts).abs().sum(dim=-1))
...
class Unsupervised_FF(nn.Module):
    def __init__(self, n_layers: int = 4, n_neurons=500, input_size: int = 28 * 28, n_epochs: int = 100,
        ...
        self.last_layer = nn.Linear(in_features=32, out_features=n_classes, bias=bias)
        self.to(device)
